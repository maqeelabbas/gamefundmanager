...
public class ExpenseService : IExpenseService
{
    ...
    public async Task<ApiResponse<IEnumerable<ExpenseDto>>> GetAllExpensesPaidByUserAsync(Guid userId)
    {
        // Get all expenses paid by this user from the repository
        var expenses = await _expenseRepository.GetUserExpensesAsync(userId);
        
        if (expenses == null)
            return ApiResponse<IEnumerable<ExpenseDto>>.SuccessResponse(new List<ExpenseDto>());
