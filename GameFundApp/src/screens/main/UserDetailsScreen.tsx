...
const UserDetailsScreen: React.FC = () => {
  ...
  const validateAuthToken = useCallback(async () => {
    ...
    try {
      ...
      if (isExpired) {
        ...
        if (!refreshed) {
          console.log(
            "Token validation failed, token may be expired and refresh failed"
          );
          return null;
        }
      }
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      console.log(
        `Loading user data (attempt ${retryCountRef.current + 1}/${maxRetries})`
      );

      // Safety check to prevent excessive retries across component re-renders
      if (retryCountRef.current >= maxRetriesThreshold) {
        console.warn(
          `🛑 Reached maximum retry threshold (${maxRetriesThreshold}), stopping retries`
        );
        setProfile((prev) => ({
          ...prev,
          loading: false,
          error: new Error(
            "Unable to authenticate after multiple attempts. Please try logging in again."
          ),
        }));
        return;
      }
      ...
      try {
        ...
        try {
          ...
          profileData.groups = userGroups.map((g) => ({
            id: g.id,
            name: g.name,
          }));
          ...
          try {
            ...
            const userContributions =
              await contributionService.getSpecificUserContributions(userId);

            if (userContributions && userContributions.length > 0) {
              profileData.profileStats.totalContributions =
                userContributions.reduce((sum, c) => sum + c.amount, 0);
          ...
          try {
            ...
            if (userExpenses && userExpenses.length > 0) {
              ...
              const recentExpenses = userExpenses.slice(0, 3).map((e) => ({
                id: e.id,
                action: "Paid Expense",
          ...
          profileData.recentActivity.sort(
          ...
          if (isAuthError && retryCountRef.current < maxRetries) {
            ...
            console.log(
              `Authentication error, retrying (${retryCountRef.current}/${maxRetries})...`
            );
            ...
            setProfile((prev) => ({
              ...
              error:
                error instanceof Error
                  ? error
                  : new Error("Failed to load user profile data"),
            }));
          }
        }
      } catch (error) {
    ...
    <StyledView className="flex-1 bg-background">
      ...
      {profile.error && (
        <StyledView className="flex-1 justify-center items-center p-4">
          <StyledText className="text-red-500 text-lg mb-3">
      ...
      {!profile.loading && !profile.error && user && (
        <StyledScrollView className="flex-1">
          ...
          <StyledView className="bg-white p-6 items-center border-b border-gray-100">
            ...
            <StyledText className="text-sm text-gray-500 mb-2">
              {user.role}
            </StyledText>
          </StyledView>
          {/* User Stats */}
          <StyledView className="flex-row justify-between p-4 bg-white border-b border-gray-100">
            <StyledView className="items-center">
              <StyledText className="text-primary font-bold text-lg">
                {"€"} {user.profileStats.totalContributions}
              </StyledText>
              <StyledText className="text-text text-xs">Contributed</StyledText>
            </StyledView>
            <StyledView className="items-center">
              <StyledText className="text-red-500 font-bold text-lg">
                {"€"} {user.profileStats.expensesPaid}
              </StyledText>
            ...
            <StyledView className="items-center">
              <StyledText className="text-accent font-bold text-lg">
                {user.profileStats.activeGroups}
              </StyledText>
              <StyledText className="text-text text-xs">
                Active Groups
              </StyledText>
          ...
          <StyledView className="bg-white p-4 mb-4">
            ...
            {user.groups.length > 0 ? (
              user.groups.map((group, index) => (
                <StyledView
                  key={index}
                  className="flex-row justify-between items-center mb-3 py-2 border-b border-gray-100"
                >
                ...
                >
                  <StyledView className="flex-row justify-between">
                    ...
                    {activity.amount && (
                      <StyledText
                        className={`font-bold ${
                          activity.action.includes("Expense") ||
                          activity.action === "Paid Expense"
                            ? "text-red-500"
                            : "text-primary"
                        }`}
                      >
                  ...
                  <StyledText className="text-text text-xs mt-1">
                    {activity.group}
                  </StyledText>
