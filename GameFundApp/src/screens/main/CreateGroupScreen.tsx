    ...
    <StyledView className="flex-1 bg-background">
      ...
      <StyledView className="bg-primary p-6 pt-12">
        <StyledView className="flex-row items-center">
          ...
          <StyledText className="text-white text-xl font-bold">Create New Group</StyledText>
        </StyledView>
      </StyledView>
      ...
      <StyledScrollView className="flex-1 p-4">
        <StyledView className="bg-white p-6 rounded-xl shadow-sm mb-4">
          ...
          <StyledText className="text-sm font-medium text-text mb-2">Sport Type *</StyledText>
          <StyledView className="flex-row flex-wrap mb-4">
            {sportTypes.map(sport => (
              <StyledTouchableOpacity
                key={sport}
                className={`border rounded-full px-4 py-2 mr-2 mb-2 ${
                  selectedSport === sport ? 'border-primary bg-primary bg-opacity-10' : 'border-gray-300'
                }`}
                onPress={() => setSelectedSport(sport)}
              >
                <StyledText className={`${selectedSport === sport ? 'text-primary font-medium' : 'text-text'}`}>
                  {sport}
                </StyledText>
              </StyledTouchableOpacity>
            ))}
          ...
          {/* Description Input */}
          <StyledText className="text-sm font-medium text-text mb-2">Group Description</StyledText>
          <StyledTextInput
