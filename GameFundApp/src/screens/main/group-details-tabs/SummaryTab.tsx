...
export const SummaryTab: React.FC<SummaryTabProps> = ({
  ...
  const totalContributionsAmount = contributions && Array.isArray(contributions) 
    ? contributions.reduce((sum, contribution) => sum + contribution.amount, 0) 
    : getTotalContributions();
  
  // Use the group's currency or default to USD
    ...
    <StyledView className="px-4 mb-6">
      ...
      <StyledView className="bg-white rounded-xl p-4 shadow-sm mb-4">
        ...
        <StyledText className="text-gray-700 mb-2">
          {groupCurrency} {totalContributionsAmount.toFixed(2)} of {targetAmount.toFixed(2)} {groupCurrency} collected
        </StyledText>
        ...
        <StyledView className="flex-row justify-between">
          <StyledText className="text-gray-500 text-sm">{getProgressPercentage()}% Complete</StyledText>
          <StyledText className="text-gray-500 text-sm">{groupCurrency} {remainingAmount.toFixed(2)} remaining</StyledText>
        </StyledView>
      </StyledView>
