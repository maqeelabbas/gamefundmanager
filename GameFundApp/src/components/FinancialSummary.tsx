...
import { StyledView, StyledText, StyledTouchableOpacity, StyledImage } from '../utils/StyledComponents';
...
export const FinancialSummary: React.FC<FinancialSummaryProps> = ({ 
  ...
  const loadingExpenses = providedLoadingExpenses !== undefined ? providedLoadingExpenses : expensesLoading;
  const loadingContributions = providedLoadingContributions !== undefined ? providedLoadingContributions : contributionsLoading;
  ...
  const fetchContributions = providedFetchContributions || fetchContributionsLocal;
    ...
    <StyledView>
      ...
      <StyledView className="flex-row justify-between mb-6">
        ...
        <StyledView className="bg-white p-4 rounded-xl shadow-sm w-[31%]">
          ...
          <StyledText 
            className={`${balance >= 0 ? 'text-primary' : 'text-red-500'} text-lg font-bold`}
          >
      ...
      <StyledView className="mb-6">
        <StyledText className="text-lg font-bold text-text mb-3">Recent Expenses</StyledText>
        
        {loadingExpenses ? (
          <StyledView className="bg-white p-4 rounded-xl shadow-sm items-center">
            <StyledText className="text-gray-500">Loading expenses...</StyledText>
          </StyledView>
        ) : expensesError ? (
          <StyledView className="bg-white p-4 rounded-xl shadow-sm items-center">
          ...
          <StyledView className="bg-white p-4 rounded-xl shadow-sm items-center">
            <StyledText className="text-gray-500">No expenses recorded</StyledText>
          </StyledView>
        )}
        
        <StyledTouchableOpacity className="mb-3" onPress={() => {
          if (onViewAllExpenses) {
            onViewAllExpenses();
          } else {
            Alert.alert('View All Expenses', 'This would open the full expenses list.');
          }
        }}>
          <StyledText className="text-primary text-center">View All Expenses</StyledText>
        </StyledTouchableOpacity>
      </StyledView>
      
      {/* Recent Contributions */}
      <StyledView className="mb-4">
        <StyledText className="text-lg font-bold text-text mb-3">Recent Contributions</StyledText>
        
        {loadingContributions ? (
          <StyledView className="bg-white p-4 rounded-xl shadow-sm items-center">
            <StyledText className="text-gray-500">Loading contributions...</StyledText>
          </StyledView>
        ) : contributionsError ? (
          <StyledView className="bg-white p-4 rounded-xl shadow-sm items-center">
          ...
          <StyledView className="bg-white p-4 rounded-xl shadow-sm items-center">
            <StyledText className="text-gray-500">No contributions recorded</StyledText>
          </StyledView>
        )}
        ...
        <StyledTouchableOpacity className="mb-3" onPress={() => {
          if (onViewAllContributions) {
            ...
            Alert.alert('View All Contributions', 'This would open the full contributions list.');
          }
        }}>
          <StyledText className="text-primary text-center">View All Contributions</StyledText>
        </StyledTouchableOpacity>
      </StyledView>
    </StyledView>
  );
